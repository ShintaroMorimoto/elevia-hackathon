...
import { ArrowLeftIcon, CalendarIcon, ChevronDownIcon, ChevronRightIcon, PlusIcon, ComponentPlaceholderIcon } from "@radix-ui/react-icons"
...
export default function WorldTravelPlanPage() {
  const [expandedYears, setExpandedYears] = useState<Set<number>>(new Set([2024]))
  const [tasks, setTasks] = useState<Task[]>([
    // 2024年 - 準備期間
    { id: "2024", title: "準備・計画の年", completed: false, type: "yearly", year: 2024 },
    {
      id: "2024-1",
      title: "世界一周ルートの詳細計画",
      completed: true,
    ...
    // 2025年 - 本格準備
    { id: "2025", title: "本格準備の年", completed: false, type: "yearly", year: 2025 },
    {
      id: "2025-1",
      title: "航空券の予約（早期割引活用）",
      completed: false,
    ...
    // 2026年 - 出発準備
    { id: "2026", title: "出発準備・実行の年", completed: false, type: "yearly", year: 2026 },
    {
      id: "2026-1",
      ...
      <header className="bg-white border-b border-gray-200 px-4 py-3">
        <div className="flex items-center">
          ...
          <div className="flex-1">
            <h1 className="text-lg font-bold text-gray-900">世界一周旅行を実現する</h1>
      ...
      <main className="p-4">
        ...
        <div className="space-y-4">
          {yearlyTasks.map((yearTask) => {
            ...
            const yearRegularTasks = regularTasks.filter((t) => t.year === year)
            const yearCompletedCount = yearRegularTasks.filter((t) => t.completed).length
            const yearTotalCount = yearRegularTasks.length
            const yearProgress = yearTotalCount > 0 ? Math.round((yearCompletedCount / yearTotalCount) * 100) : 0
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          {yearTotalCount > 0 && (
                            <p className="text-sm text-gray-600">
                              {yearCompletedCount}/{yearTotalCount} 完了 ({yearProgress}%)
                            </p>
                          )}
                        </div>
                      </div>
                  ...
                  {isExpanded && yearRegularTasks.length > 0 && (
                    <div className="border-t border-gray-200">
                      {yearRegularTasks.map((task) => (
                        <div key={task.id} className="p-4 pl-12 border-b border-gray-100 last:border-b-0">
                              ...
                              <div className="flex-1">
                                ...
                                <div className="flex items-center space-x-2 text-xs">
                                  <Badge variant="outline" className={getCategoryColor(task.category)}>
                                    ...
                                    {task.category === "documentation" && "書類"}
