...
export const goals = pgTable('goals', {
  ...
  progressPercentage: decimal('progress_percentage', { precision: 5, scale: 2 }).default('0'), // 全体進捗率
  createdAt: timestamp('created_at').notNull().defaultNow(),
  updatedAt: timestamp('updated_at').notNull().defaultNow(),
}, (table) => ({
...
export const yearlyOkrs = pgTable('yearly_okrs', {
  id: uuid('id').primaryKey().defaultRandom(),
  goalId: uuid('goal_id').notNull().references(() => goals.id, { onDelete: 'cascade' }),
  objective: text('objective').notNull(), // 年次の目標（Objective）
  targetYear: integer('target_year').notNull(),
  sortOrder: integer('sort_order').notNull().default(0),
  progressPercentage: decimal('progress_percentage', { precision: 5, scale: 2 }).default('0'), // OKR進捗率
  createdAt: timestamp('created_at').notNull().defaultNow(),
  updatedAt: timestamp('updated_at').notNull().defaultNow(),
}, (table) => ({
...
export const quarterlyOkrs = pgTable('quarterly_okrs', {
  id: uuid('id').primaryKey().defaultRandom(),
  yearlyOkrId: uuid('yearly_okr_id').notNull().references(() => yearlyOkrs.id, { onDelete: 'cascade' }),
  ...
  yearlyOkrIdIdx: index('idx_quarterly_okrs_yearly_okr_id').on(table.yearlyOkrId),
...
export const keyResults = pgTable('key_results', {
  id: uuid('id').primaryKey().defaultRandom(),
  yearlyOkrId: uuid('yearly_okr_id').references(() => yearlyOkrs.id, { onDelete: 'cascade' }),
  quarterlyOkrId: uuid('quarterly_okr_id').references(() => quarterlyOkrs.id, { onDelete: 'cascade' }),
  description: text('description').notNull(),
  targetValue: decimal('target_value', { precision: 10, scale: 2 }).notNull(),
  currentValue: decimal('current_value', { precision: 10, scale: 2 }).default('0'),
  unit: varchar('unit', { length: 50 }), // %, 個, 円 など
  achievementRate: decimal('achievement_rate', { precision: 5, scale: 2 }).default('0'), // 達成率(0-100%)
  sortOrder: integer('sort_order').notNull().default(0),
  createdAt: timestamp('created_at').notNull().defaultNow(),
  updatedAt: timestamp('updated_at').notNull().defaultNow(),
}, (table) => ({
  yearlyOkrIdIdx: index('idx_key_results_yearly_okr_id').on(table.yearlyOkrId),
  quarterlyOkrIdIdx: index('idx_key_results_quarterly_okr_id').on(table.quarterlyOkrId),
...
export const chatMessages = pgTable('chat_messages', {
  ...
  senderTypeCheck: check('sender_type_check', sql`sender_type IN ('user', 'ai')`),
}));
