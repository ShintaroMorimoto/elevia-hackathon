 ...
 * AI深掘り対話:
   ...
   * AIによるロードマップ生成機能:
     * 入力された情報に基づき、AIが達成までの年単位・3か月単位の OKR を自動で作成する。
   * 計画の編集機能:
     * AIが生成した年・3か月の OKR を、ユーザーが自由に追加・編集・削除できる。
   * メイン画面（階層リストビュー）:
	   * スマホに最適化されたシンプルなリスト形式の画面
	   * 最終目標 → 年次 OKR → 3か月 OKR へとドリルダウンして確認できる


#### 要件
上記の要求をもとに作ったもの
1. 概要・目的
- ユーザーが掲げた壮大な夢 (Objective) に対し、AIが達成までのロードマップ (年/3か月の OKR) を自動生成し、長期的な視点での目標達成を支援するプランニングツール。
1. システム利用ユーザー (アクター)
 ...
 * 3.2.3. 目標詳細表示（階層リストビュー）:
   ...
   * 各「年次 OKR」をタップすると、その年に紐づく「3か月 OKR」のリストへとドリルダウン（画面遷移またはアコーディオン展開）して表示される。
 ...
 * 3.4.1. AIによるパーソナライズされた OKR 生成:
   ...
   * 処理: 上記の全情報を基に、ユーザーに最適化されたロードマップ（年/3か月のOKR）の草案を生成する。
3.5. OKR 管理機能
 * 3.5.1. OKR の手動での追加・編集・削除:
   * ユーザーは、AIが生成した年次・月次の各 OKR のテキストを編集できる。
   * ユーザーは、不要な OKR を削除できる。
   * ユーザーは、任意の位置に新しい OKR を手動で追加できる。
 * 3.5.2. OKR のステータス変更:
   * 各 OKR には「完了」を示すチェックボックスを設ける。
   * ユーザーがチェックを入れると、その OKR は完了済みとなる。
   * OKR の完了状態は、目標全体の進捗プログレスバーに自動で反映される。
1. 非機能要件
システムの品質や制約に関する要件を定義します。
 * 4.1. ユーザーインターフェース / ユーザー体験（UI/UX）
   * デザインはスマートフォンでの利用を最優先（スマホファースト）とする。
   * 操作に迷わない、シンプルで直感的な画面構成とする。
   * 情報の階層（目標 > 年 > 月）がわかりやすく表現されていること。
 * 4.2. パフォーマンス
   * 通常の画面遷移は2秒以内に完了すること。
