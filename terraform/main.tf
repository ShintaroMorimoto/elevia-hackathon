...
# Updated: Added permission debugging to GitHub Actions workflow
terraform {
...
resource "google_cloud_run_v2_service" "app" {
  ...
  template {
    ...
    containers {
      ...
      env {
        name  = "NEXTAUTH_URL"
        value = "https://${var.custom_domain != "" ? var.custom_domain : "${var.app_name}-${random_id.suffix.hex}-${substr(md5(var.project_id), 0, 8)}.a.run.app"}"
      }
    }
